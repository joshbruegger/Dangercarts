...
# Convert to m/s
scoreboard players operation out math_sqrt *= dc.vars.2 dc_num
scoreboard players operation out math_sqrt /= dc.vars.100 dc_num
scoreboard players operation out math_sqrt -= dc.vars.1 dc_num
...
execute if score out math_sqrt > dc.vars dc_low as @e[type=!#dangercarts:not_mob,distance=0..1.5,tag=!dc_passenger] run summon area_effect_cloud ~ ~320 ~ {Duration:3,Tags:["dc_slime"],Passengers:[{id:"minecraft:slime",Tags:["dc_actual_slime"],Silent:1b,Invulnerable:1b,Size:1,CustomName:'{"text":"a fast minecart"}',ActiveEffects:[{Id:14b,Amplifier:1b,Duration:999999,ShowParticles:0b}],Attributes:[{Name:generic.attack_damage,Base:0}]}]}

# high damage if speed is greater than high threshold
execute if score out math_sqrt > dc.vars dc_high run execute as @s run function dangercarts:deal_damage with storage dangercarts:config dc_high_damage

# medium damage if speed is greater than medium threshold
execute if score out math_sqrt > dc.vars dc_medium unless score out math_sqrt > dc.vars dc_high run execute as @s run function dangercarts:deal_damage with storage dangercarts:config dc_medium_damage

# low damage if speed is greater than low threshold
execute if score out math_sqrt > dc.vars dc_low unless score out math_sqrt > dc.vars dc_medium run execute as @s run function dangercarts:deal_damage with storage dangercarts:config dc_low_damage
